...
pub(crate) fn spawn_qa_login_pipe_reader(app: AppHandle, pipe_path: PathBuf) {
    tauri::async_runtime::spawn(async move {
        ...
        if let Some(recovery_secret) = request.recovery_secret {
            let state = app.state::<CoreRuntimeState>();
            if let Err(message) =
                wait_for_qa_recovery_prompt(&app, state.inner(), super::QA_RECOVERY_PROMPT_TIMEOUT)
                    .await
            {
                record_qa_login_failure(&app, &message).await;
                return;
...
mod qa_tests {
    use super::{
        QaControlCommand, parse_qa_control_pipe_line, parse_qa_login_pipe_payload,
        qa_recovery_prompt_is_available,
    };
    use crate::commands::qa_window_title_string;
