...
describe("desktop release scripts", () => {
  ...
  test("app keeps Tauri send completion listener mounted and gates events with a pending ref", () => {
    ...
    expect(source).toMatch(
      /useEffect\(\(\) => \{[\s\S]*if \(!isTauriRuntime\(\)\) \{[\s\S]*runInBackground\([\s\S]*desktopEventPort[\s\S]*\.listenCoreEvents[\s\S]*qaSendPending\.current[\s\S]*qaSendCompletionStatusFromCoreEvent[\s\S]*setQaSendStatus\(eventStatus\);[\s\S]*\}, \[\]\);/
    );
    expect(source).toMatch(
