...
mod tests {
    ...
    fn login_success_is_not_blocked_by_optional_account_hydration() {
        let login_body = crate::account::test_source::item_body(
        ...
        let promotion = crate::account::test_source::item_body(
    ...
    fn async_account_hydration_is_generation_gated() {
        ...
        assert!(
            actor_source.contains("AccountHydrationLoaded {"),
            "optional account hydration should return through the actor mailbox"
        );
        assert!(
            profile_source.contains("generation != self.account_hydration_generation"),
            "stale account hydration from an old session must be dropped before reducer actions are sent"
        );
        assert!(
            profile_source.contains("fn invalidate_account_hydration(&mut self)"),
