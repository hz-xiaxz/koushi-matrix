...
fn index_lists_rooms_with_status_counts_and_links() {
    let mut manifest = ExportManifest::new(
        ManifestScope::Space { id: "!s:example.org".to_owned() },
        HistoryExportRange::AllAvailable,
        "Lab & Friends".to_owned(),
    );
    let counts = HistoryExportRoomCounts { exported_events: 12, attachments_total: 3, attachments_failed: 1, ..Default::default() };
    manifest.upsert_room("!a:x", "General", "General (0000000a)", ManifestRoomStatus::Completed, counts);
    manifest.upsert_room("!b:x", "Secret <b>", "Secret _b_ (0000000b)", ManifestRoomStatus::Skipped, HistoryExportRoomCounts::default());
    manifest.upsert_room("!c:x", "Broken", "Broken (0000000c)", ManifestRoomStatus::Failed, HistoryExportRoomCounts::default());
    let page = String::from_utf8(render_index_page(&manifest, &labels(), 1_758_758_400_000, "UTC")).unwrap();
    assert!(page.contains("<title>Lab &amp; Friends</title>"), "{page}");
    assert!(page.contains("<a href=\"rooms/General%20%280000000a%29/index.html\">General</a>"), "{page}");
...
fn period_range_is_shown_with_inclusive_end_date() {
    let manifest = ExportManifest::new(
        ManifestScope::Room { id: "!a:x".to_owned() },
        HistoryExportRange::Period { start_ms: 1_756_652_400_000, end_exclusive_ms: 1_759_244_400_000, time_zone: "Asia/Tokyo".to_owned() },
        "General".to_owned(),
    );
    let page = String::from_utf8(render_index_page(&manifest, &labels(), 1_758_758_400_000, "Asia/Tokyo")).unwrap();
