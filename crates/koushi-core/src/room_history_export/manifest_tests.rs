...
fn manifest() -> ExportManifest {
    let mut manifest =
        ExportManifest::new(scope(), HistoryExportRange::AllAvailable, "Lab".to_owned());
...
fn fresh_when_no_manifest() {
    assert_eq!(
        match_manifest(None, &scope(), &HistoryExportRange::AllAvailable),
        ManifestMatch::Fresh
    );
}
...
fn mismatch_on_other_space_or_range() {
    ...
    assert_eq!(
        match_manifest(Some(&bytes), &other, &HistoryExportRange::AllAvailable),
    ...
    assert_eq!(
        match_manifest(Some(&bytes), &scope(), &period),
        ManifestMatch::Mismatch
    );
}
...
fn mismatch_on_unknown_format_version_or_corrupt_json() {
    ...
    assert_eq!(
        match_manifest(Some(&newer), &scope(), &HistoryExportRange::AllAvailable),
        ManifestMatch::Mismatch
    );
    ...
    assert_eq!(
        match_manifest(Some(&foreign), &scope(), &HistoryExportRange::AllAvailable),
        ManifestMatch::Mismatch
    );
    assert_eq!(
        match_manifest(
...
fn upsert_updates_in_place_and_keeps_order() {
    ...
    let ids: Vec<_> = manifest
        .rooms
        .iter()
        .map(|room| (room.room_id.as_str(), room.status))
        .collect();
    assert_eq!(
        ids,
        vec![
            ("!a:example.org", ManifestRoomStatus::Failed),
            ("!b:example.org", ManifestRoomStatus::Pending)
        ]
    );
    assert_eq!(
        manifest.room("!b:example.org").unwrap().folder,
        "B (00000002)"
    );
}
