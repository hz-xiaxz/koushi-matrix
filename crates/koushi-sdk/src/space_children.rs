...
//!
//! What the server withholds stays withheld: a child the Space advertises that
//! `/hierarchy` did not describe is reported with an unknown membership and no
//! name, never probed for through another endpoint.
...
pub async fn matrix_space_children_projection(
    ...
    let mut entries: Vec<MatrixSpaceChildEntry> = room_list
        .rooms()
        .await
        .into_iter()
        .map(|room| MatrixSpaceChildEntry {
            ...
            joined_members: room.num_joined_members,
        })
        .collect();
