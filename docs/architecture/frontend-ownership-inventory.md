...
- **App request-ref families:** Phases 4.1–4.3a classify and bound settings, diagnostics and Space-member panel/load demand. Phase 4.3b keeps distinct mounted-panel/shared-workflow renderer epochs while making Tauri return exact Rust open/search/close projections. Room/Space navigation and Space-member invite execution/cancel/role remain separate investigation families.
...
4. **Remaining App promise request refs** — Phases 4.1–4.3b resolve settings, diagnostics, Space-member panel/load and invite-search lifetimes. Navigation and Space-member invite execution/cancel/role paths may still duplicate Rust request/demand generations, but each family needs an equivalence test before deletion.
...
| Identify duplicated Rust/TS semantics | **Complete as inventory; partial as migration:** #708 thread-root duplication and Phase 3 view-owned acknowledgement retry are removed; Phases 4.1–4.3b classify/bound renderer demand and repair invite-search convergence, while remaining request families and alias/caption sequencing retain proof gates. |
| Migrate selected high-value leaf owners incrementally | **Partial overall:** PR #683 migrated invite/mention query admission, Phase 1 thread roots, Phase 3 ACK delivery, Phases 4.1–4.2 settings/diagnostics, Phase 4.3a bounded Space-member demand and Phase 4.3b converged invite search; other leaves remain. |
| One documented semantic owner per migrated subsystem | **Partial overall; complete through Phase 4.3b:** Rust owns invite query/workflow, mention, thread-root, room-settings, diagnostics and Space-member semantics; renderer epochs own only mounted/cross-surface intent, and bounded adapters own delivery/dedupe/convergence. |
...
| Focused transition/teardown/projection-equivalence tests | **Partial overall; complete through Phase 4.3b:** deterministic deferred query/settings/diagnostics/Space-member tests cover exact Tauri convergence/deadline/lag, cross-surface replacement, spinner settlement, account replacement, bounded demand and stale no-apply. |
