...
- **App request-ref families:** Phase 4.1 proves room/Space settings load markers/request epochs are renderer-specific panel-demand owners. Phase 4.2 proves the diagnostics open-intent epoch is renderer-specific because its DTO is outside AppState/appStore and has no Rust request/generation correlation. Room/Space navigation, Space-member open/invite/cancel/role and Space invite search remain separate investigation families. Button/in-flight refs and local dialog lifetimes remain renderer presentation unless a duplicated semantic transition is proven.
...
4. **Remaining App promise request refs** — Phase 4.1 resolved room/Space settings refs and Phase 4.2 resolved diagnostics open intent as necessary renderer fences. Navigation, Space-member and invite-search paths may still duplicate Rust request/demand generations, but each command-response family needs an equivalence test before deletion.
...
| Identify duplicated Rust/TS semantics | **Complete as inventory; partial as migration:** #708 thread-root duplication and Phase 3 view-owned acknowledgement retry are removed; Phases 4.1–4.2 classify settings and diagnostics refs as renderer-specific, while remaining request families and alias/caption sequencing retain task-level proof gates. |
| Migrate selected high-value leaf owners incrementally | **Partial overall:** PR #683 migrated invite/mention query admission, Phase 1 migrated thread-root lifecycle/placement, Phase 3 migrated acknowledgement delivery, and Phases 4.1–4.2 resolve settings/diagnostics families without forced deletion; other request-ref and mutation leaves remain. |
| One documented semantic owner per migrated subsystem | **Partial overall; complete through Phase 4.2:** Rust owns invite/mention, thread-root, room-settings and diagnostic-content semantics; the renderer owns settings panel demand, diagnostics dialog intent and ACK evidence, the bounded ACK adapter owns only pre-Core delivery, and Rust owns post-acceptance ACK semantics. |
...
| Remove corresponding TS semantic state after cutover | **Partial overall:** invite/mention queue keys, #708 projection maps/pruning/placement, and TimelineView acknowledgement retry refs/timers are removed; Phase 4.1 settings and Phase 4.2 diagnostics refs are documented renderer presentation, while retained mutation and other request fences remain pending proof. |
...
| Focused transition/teardown/projection-equivalence tests | **Partial overall; complete through Phase 4.2:** deterministic deferred invite/main/thread query, settings-panel and diagnostics-dialog tests cover monotone snapshots, adversarial/equal-generation settlement, panel replacement, pending dedupe, failure/reopen and stale no-reopen; future leaves require their own RED/GREEN tests. |
