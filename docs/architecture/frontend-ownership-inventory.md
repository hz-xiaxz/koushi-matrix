...
- **Direct Tauri imports:** seven production modules import `@tauri-apps/*`: `App.tsx`; `app/useDesktopAttentionEffects.ts`; `backend/appRuntime.ts`, `backend/client.ts`, `backend/tauriTimelineTransport.ts`, and `backend/tauri/linkMediaPort.ts`; plus the deferred `domain/desktopNotification.ts`. External-link and media URL/save imports are now adapter-owned and the domain guard forbids their return.
- **Neutral API/port dependency:** `DesktopApi` lives in `backend/desktopApi.ts`; `TauriDesktopApi` and `BrowserFakeApi` implement it; `appRuntime` selects it. `LinkMediaPort` has browser/Tauri implementations selected by `linkMediaRuntime`. All runtime checks import the leaf `backend/runtimeEnvironment.ts`; the timeline module no longer exports the predicate. Later Phase 2B families still isolate notification, window/dialog, and event-subscription imports.
