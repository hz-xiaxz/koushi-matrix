...
| `latestTextMutationQueueRef` main/thread caption lanes | Per-staged-item mutation lifetime | **Unresolved.** Rust owns staged DTOs but revision/latest-intent equivalence requires separate proof. | Existing A/B/A/invalidation coverage. | **Phase 5B candidate.** |
...
- **Remaining mutation queue:** Phase 5A proves local-alias lanes are bounded renderer-specific pre-terminal autosave sequencing. Main/thread staged-caption `run`/`invalidate` users remain for Phase 5B. Invite and mention query keys were removed by merged PR #683.
...
2. **Remaining caption mutation ordering** — Phase 5A explicitly retains alias autosave sequencing because Tauri returns pre-terminal and browser generations are equal. Staged-caption writes still need Phase 5B proof. Invite/mention query ordering is already migrated.
...
- **Current result:** merged PR #683 migrated invite target search and main/thread mention query admission to existing Rust request/generation state plus `appStore`; Phase 5A proves alias serialization renderer-specific and bounded; main/thread caption mutation serialization remains pending.
...
The shipped invite/mention query leaf touches none of those owners; Phase 5A documents alias serialization as a separate bounded renderer transport/result owner, while caption sequencing remains unresolved.
...
| Identify duplicated Rust/TS semantics | **Complete as inventory; partial as migration:** thread/ACK duplication is removed; Phases 4.1–4.3b classify/bound demand, 4.3c–4.3e remove invite/cancel/role latest-click authority, 4.4 proves navigation epochs renderer-specific, and 5A proves alias sequencing renderer-specific. Caption gates remain. |
...
| One documented semantic owner per migrated subsystem | **Partial overall; complete through Phase 5A:** Rust owns submitted navigation, aliases, invite query/workflow/execution/cancellation/role and other migrated semantics; renderer owners are limited to pre-submit/mounted/cross-surface intent, local failure or bounded pre-terminal alias autosave transport/results. |
...
| Remove corresponding TS semantic state after cutover | **Partial overall:** query keys, thread/ACK state, unbounded Space-member load Map/Set and invite latest-request ref are removed; cancel/role refs are narrowed to failure presentation; navigation and alias sequencing are proven renderer-specific. Caption fences await proof. |
...
| Focused transition/teardown/projection-equivalence tests | **Partial overall; complete through Phase 5A:** deferred tests cover convergence, replacement, bounded demand, rapid duplicate settlement, navigation ordering, alias latest autosave/rejected-tail continuation, retry and panel close/reopen. |
