...
| `backend/desktopApi.ts::DesktopApi`, `backend/client.ts::TauriDesktopApi`, `backend/browserFakeApi.ts`, `backend/appRuntime.ts`, and direct `@tauri-apps/*` imports | Page/runtime adapter lifetime; fake implementation exists only in browser/test mode | **Renderer transport/adapter plus test-backend mirror.** The neutral interface has no implementation dependency; Tauri and browser adapters implement it, and `appRuntime` alone selects one. | IPC names and Rust DTOs remain authoritative. Composition-root tests prove exactly one adapter is constructed per runtime. This structural direction change creates no Matrix/product state owner. | **Phase 2A complete as structural isolation; not a semantic migration.** Phase 2B still moves direct platform imports behind existing-operation ports family by family. |
...
- **Neutral API dependency:** `DesktopApi` is declared in `backend/desktopApi.ts`; `TauriDesktopApi` and `BrowserFakeApi` implement it; `backend/appRuntime.ts` alone selects through the existing `tauriTimelineTransport::isTauriRuntime` predicate. Phase 2A changed only this contract/selection dependency direction. Phase 2B still moves direct platform imports behind existing-operation ports.
