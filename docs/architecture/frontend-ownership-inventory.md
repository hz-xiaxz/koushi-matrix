...
| `App.tsx` remaining room/Space navigation and Space-member cancel/role request refs | Page/dialog/operation lifetime; manually incremented on navigation/close | **Renderer presentation / transport fence** around async command responses. Rust request IDs, demand generations and StateDelta ordering are authoritative. | Delayed promise result is ignored when local request ref/selection no longer matches. | **Investigate derive/delete by family.** Cancel/role epochs guard renderer-local failure presentation but share the known latest-click inversion. |
...
- **App request-ref families:** Phases 4.1–4.3b classify/bound settings, diagnostics, Space-member panel/load and invite search. Phase 4.3c removes invite execution's latest-click ref in favor of Rust first-admitted operation authority. Room/Space navigation and Space-member cancel/role remain separate investigation families.
...
4. **Remaining App promise request refs** — Phases 4.1–4.3c resolve settings, diagnostics, Space-member panel/load/search and invite execution. Navigation and Space-member cancel/role paths remain; both mutation refs share a known latest-click/first-admitted inversion but guard local failure UI.
...
- **Risk:** cancel/role refs still combine stale transport-failure presentation with the same latest-click inversion and require separate fixes.
...
| Identify duplicated Rust/TS semantics | **Complete as inventory; partial as migration:** thread/ACK duplication is removed; Phases 4.1–4.3b classify/bound renderer demand, and Phase 4.3c removes invite latest-click authority; cancel/role/navigation and alias/caption retain proof gates. |
| Migrate selected high-value leaf owners incrementally | **Partial overall:** query admission, thread roots and ACK delivery migrated; Phases 4.1–4.3b classify/bound renderer demand and converge invite search; Phase 4.3c removes invite latest-click authority. Other leaves remain. |
| One documented semantic owner per migrated subsystem | **Partial overall; complete through Phase 4.3c:** Rust owns invite query/workflow/execution, mention, thread-root, room-settings, diagnostics and Space-member semantics; renderer epochs own only mounted/cross-surface intent and bounded adapters own delivery/dedupe/convergence. |
...
| Remove corresponding TS semantic state after cutover | **Partial overall:** invite/mention queue keys, thread projection maps, TimelineView ACK retry refs/timers, unbounded Space-member load Map/Set and Space invite latest-request ref are removed; documented presentation refs remain, while cancel/role/navigation/mutation fences await proof. |
...
| Focused transition/teardown/projection-equivalence tests | **Partial overall; complete through Phase 4.3c:** deferred tests cover Tauri convergence/deadline/lag, cross-surface/account replacement, bounded demand, stale no-apply and rapid duplicate first-admitted invite settlement. |
