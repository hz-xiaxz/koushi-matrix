...
| `backend/desktopApi.ts::DesktopApi`, link/media and desktop-attention ports, concrete browser/Tauri adapters, composition modules, and remaining direct `@tauri-apps/*` imports | Page/runtime adapter lifetime; browser implementations exist only outside native Tauri | **Renderer transport/platform adapters plus test-backend mirror.** Neutral interfaces have no implementation dependency; composition modules select the concrete adapter. | IPC names and Rust DTOs remain authoritative. Composition tests prove runtime selection; link/media and attention ports own stateless platform operations, not Matrix/product state. | **Phases 2A, 2B1 and 2B2 complete as structural isolation; not semantic migration.** Later Phase 2B families still isolate window/dialog and event-subscription imports. |
...
- **Direct Tauri imports:** six production modules import `@tauri-apps/*`: `App.tsx`; `backend/appRuntime.ts`, `backend/client.ts`, `backend/tauriTimelineTransport.ts`, `backend/tauri/linkMediaPort.ts`, and `backend/tauri/desktopAttentionPort.ts`. Domain and app-hook guards forbid platform imports; remaining non-adapter imports belong to the deferred window/dialog and event-subscription families.
- **Neutral API/port dependency:** `DesktopApi` lives in `backend/desktopApi.ts`; `TauriDesktopApi` and `BrowserFakeApi` implement it; `appRuntime` selects it. `LinkMediaPort` has browser/Tauri implementations selected by `linkMediaRuntime`. `DesktopAttentionPort` is selected once by `desktopAttentionRuntime`; its Tauri factory owns current-window acquisition, notifications, sound and native badge operations while domain policy and React timing remain unchanged. All runtime checks import the leaf `backend/runtimeEnvironment.ts`. Later Phase 2B families still isolate window/dialog and event-subscription imports.
...
| Identify duplicated Rust/TS semantics | **Complete as inventory; partial as migration:** #708 thread-root duplication is removed; acknowledgement retry, request refs, and alias/caption sequencing remain named candidates with task-level proof gates. |
| Migrate selected high-value leaf owners incrementally | **Partial:** PR #683 migrated invite/mention query admission and Phase 1 #708 migrates thread-root lifecycle/placement; acknowledgement, request-ref, and mutation leaves remain. |
| One documented semantic owner per migrated subsystem | **Partial overall; complete for PR #683 and Phase 1 #708:** Rust request/generation state owns invite/mention convergence; Core service/Room actor owns thread-root lifecycle and placement. |
| Async Rust owners have cancellation/awaited settlement where required | **Partial:** #708 projection workers cancel and await on room/session teardown; Phase 3 must still settle acknowledgement reliability before this epic criterion can close. |
| Remove corresponding TS semantic state after cutover | **Partial:** invite/mention queue keys and #708 projection maps/pruning/placement are removed; acknowledgement retry and retained mutation/request fences remain pending proof. |
