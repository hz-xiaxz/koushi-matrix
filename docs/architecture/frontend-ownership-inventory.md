...
- **App request-ref families:** Phases 4.1–4.3b classify/bound renderer demand; Phase 4.3c removes invite execution's latest-click ref; Phase 4.3d narrows cancel epoch to local failure only while Rust owns settlement. Room/Space navigation and Space-member role remain separate investigation families.
...
4. **Remaining App promise request refs** — Phases 4.1–4.3d resolve settings, diagnostics, Space-member panel/load/search/invite/cancel. Navigation and Space-member role remain; role still combines local failure UI with latest-click semantic inversion.
...
- **Risk:** role ref still combines local failure presentation with latest-click inversion and requires Phase 4.3e.
...
| Identify duplicated Rust/TS semantics | **Complete as inventory; partial as migration:** thread/ACK duplication is removed; Phases 4.1–4.3b classify/bound demand, 4.3c removes invite latest-click authority and 4.3d removes it from cancellation success. Role/navigation and alias/caption retain gates. |
| Migrate selected high-value leaf owners incrementally | **Partial overall:** query/thread/ACK owners migrated; Phases 4.1–4.3b classify/bound demand; Phases 4.3c–4.3d restore Rust first-admitted invite/cancel settlement. Other leaves remain. |
| One documented semantic owner per migrated subsystem | **Partial overall; complete through Phase 4.3d:** Rust owns invite query/workflow/execution/cancellation and other migrated semantics; renderer epochs own only mounted/cross-surface intent or local failure, and bounded adapters own delivery/dedupe/convergence. |
...
| Remove corresponding TS semantic state after cutover | **Partial overall:** query keys, thread/ACK state, unbounded Space-member load Map/Set and invite latest-request ref are removed; cancel ref is narrowed to failure presentation. Role/navigation/mutation fences await proof. |
...
| Focused transition/teardown/projection-equivalence tests | **Partial overall; complete through Phase 4.3d:** deferred tests cover convergence, replacement, bounded demand, rapid duplicate first-admitted invite/cancel settlement, both failure orders, retry and panel close/reopen. |
