...
| Remaining room/Space navigation refs | Page navigation/view intent | **Renderer intent fence.** Rust request IDs and StateDelta ordering own product state. | Delayed promise result is ignored after newer local navigation. | **Investigate derive/delete by navigation family.** |
...
- **App request-ref families:** Phases 4.1–4.3b classify/bound renderer demand; Phase 4.3c removes invite execution's latest-click ref; Phases 4.3d–4.3e narrow cancel/role epochs to local failure only while Rust owns settlement. Room/Space navigation remains a separate investigation family.
...
4. **Remaining App promise request refs** — Phases 4.1–4.3e resolve settings, diagnostics and Space-member panel/load/search/invite/cancel/role. Navigation remains for separate family proof.
...
| Identify duplicated Rust/TS semantics | **Complete as inventory; partial as migration:** thread/ACK duplication is removed; Phases 4.1–4.3b classify/bound demand, and 4.3c–4.3e remove invite/cancel/role latest-click authority. Navigation and alias/caption retain gates. |
...
| One documented semantic owner per migrated subsystem | **Partial overall; complete through Phase 4.3e:** Rust owns invite query/workflow/execution/cancellation/role and other migrated semantics; renderer epochs own only mounted/cross-surface intent or local failure, and bounded adapters own delivery/dedupe/convergence. |
...
| Remove corresponding TS semantic state after cutover | **Partial overall:** query keys, thread/ACK state, unbounded Space-member load Map/Set and invite latest-request ref are removed; cancel/role refs are narrowed to failure presentation. Navigation/mutation fences await proof. |
...
| Focused transition/teardown/projection-equivalence tests | **Partial overall; complete through Phase 4.3e:** deferred tests cover convergence, replacement, bounded demand, rapid duplicate first-admitted invite/cancel/role settlement, both failure orders, retry, navigation and panel close/reopen. |
