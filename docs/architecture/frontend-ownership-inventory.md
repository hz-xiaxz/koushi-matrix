...
- **Direct Tauri imports:** six production modules import `@tauri-apps/*`: `App.tsx` (the deferred event listener only); `backend/client.ts`, `backend/tauriTimelineTransport.ts`, `backend/tauri/linkMediaPort.ts`, `backend/tauri/desktopAttentionPort.ts`, and `backend/tauri/windowDialogPort.ts`. Domain, app-hook, appRuntime, and App source guards forbid other platform imports; the remaining non-adapter import belongs only to Phase 2B4 event subscriptions.
- **Neutral API/port dependency:** `DesktopApi` lives in `backend/desktopApi.ts`; `TauriDesktopApi` and `BrowserFakeApi` implement it; `appRuntime` selects it. `LinkMediaPort` has browser/Tauri implementations selected by `linkMediaRuntime`. `DesktopAttentionPort` is selected once by `desktopAttentionRuntime`. `WindowDialogPort` exposes the existing Tauri operations without inventing a browser fallback; App retains all runtime guards and normalizations. All runtime checks import the leaf `backend/runtimeEnvironment.ts`. Phase 2B4 still isolates Core/state/menu event subscriptions.
