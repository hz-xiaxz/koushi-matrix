...
- **App request-ref families:** Phase 4.1 proves room/Space settings load markers and request epochs are renderer-specific panel-demand owners: Rust has no panel-demand projection, and equal-generation same-room results require a local intent epoch. Room/Space navigation, Space-member open/invite/cancel/role, diagnostics snapshot generation, and Space invite search remain separate investigation families. Button/in-flight refs and local dialog lifetimes remain renderer presentation unless a duplicated semantic transition is proven.
...
4. **Remaining App promise request refs** — Phase 4.1 resolved room/Space settings refs as necessary renderer panel-demand fences. Navigation, Space-member, diagnostics and invite-search paths may still duplicate Rust request/demand generations, but each command-response family needs an equivalence test before deletion.
...
| Identify duplicated Rust/TS semantics | **Complete as inventory; partial as migration:** #708 thread-root duplication and Phase 3 view-owned acknowledgement retry are removed; Phase 4.1 classifies settings refs as renderer-specific, while remaining request families and alias/caption sequencing retain task-level proof gates. |
| Migrate selected high-value leaf owners incrementally | **Partial overall:** PR #683 migrated invite/mention query admission, Phase 1 migrated thread-root lifecycle/placement, Phase 3 migrated acknowledgement delivery, and Phase 4.1 resolves the settings family without forced deletion; other request-ref and mutation leaves remain. |
| One documented semantic owner per migrated subsystem | **Partial overall; complete through Phase 4.1:** Rust owns invite/mention, thread-root and room-settings semantics; the renderer owns settings panel demand and ACK evidence, the bounded ACK adapter owns only pre-Core delivery, and Rust owns post-acceptance ACK semantics. |
...
| Focused transition/teardown/projection-equivalence tests | **Partial overall; complete through Phase 4.1:** deterministic deferred invite/main/thread query and settings-panel tests cover monotone snapshots, adversarial/equal-generation settlement, panel replacement, pending dedupe and failure/reopen; future leaves require their own RED/GREEN tests. |
