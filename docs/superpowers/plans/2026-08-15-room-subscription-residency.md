...
- PR #534 CI correction RED: the Rust job ran the residency binary in parallel with the full workspace and exposed two harness races. `lost_leave_ack_fails_closed` observed the correlated failure but counted zero diagnostics, and `pre_sync_leave_targets_replacement_manager` timed out polling the diagnostic. Both isolated post-snapshot records with `skip(snapshot_len)`, which becomes empty when the bounded diagnostic ring is already full; the install-gap probe also lacked a RoomActor processed barrier. Product residency behavior and 23 sibling tests passed.
- PR #534 CI correction GREEN: a feature-gated `RoomMessage::TestCommand` now acknowledges after the real RoomActor command handler returns, and both probes compare matching diagnostic counts before/after instead of indexing the bounded ring by vector length. Reviewer-gpt found the same positional assumption in the private-safe diagnostic test; it now compares matching `core.subscription` counts and inspects the full current matching set instead of slicing by ring position. The 25-test residency binary passed ten consecutive runs at `--test-threads=25` after each correction; core lib passed 1005/8, QA-bin passed 129, default check and diff check passed. The frontend job's separate failure was an unrelated existing release-script test exceeding its 15-second timeout after 1352 sibling tests passed; no frontend source changed.
- PR #534 second Rust RED: after the residency lane passed, `runtime_notification_settings` failed because ordinary feature-enabled integration runtimes no longer received the expected no-session Subscribe failure. The failure reproduced locally on the branch, including as a single isolated test, while the same four-test binary passed on detached `origin/main`. Root cause was #532's blanket `#[cfg(feature = "test-hooks")] session_missing=false`, originally added so the synthetic residency manager could build actors.
- PR #534 second Rust GREEN: `test_session_available` is now a per-manager field under `cfg(any(test, feature="test-hooks"))`; all normal/session-backed constructors set it false, and only the synthetic residency manager sets it true. The broad feature-level bypass is removed. `runtime_notification_settings` passed ten consecutive four-thread runs; residency passed 25/25, core lib 1005/8, QA-bin 129, default check and diff check passed.
