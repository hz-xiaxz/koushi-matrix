...
- [ ] **User Settings shared status-primitives prerequisite** — move the existing `TrustStatusRow`, `TrustActionButton`, `TrustTone`, and `DetailRow` used across the panel, Security, and Trust into one direct private shared module before either feature moves.
...
- Merged PRs: #560 (Search History pilot), #561 (shared UIA prerequisite), and #562 (Sessions).
