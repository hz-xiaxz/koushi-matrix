...
| `event_wait.rs` | `QaEventFuture`, `QaEventSource`, `QaSnapshotEventSource`, `QaEventDeadline`, `PairedEventWaitError`, paired wait; `SendFlowOutcome`, `SendQueueLocalEcho`, `SendFlowWaiter`, `BodyWaitObserver`; `wait_for_send_flow_completion`, `wait_for_send_flow_completion_with_timeout`, `send_text_expect_local_echo`, `subscribe_timeline_for_qa`, `wait_for_initial_items`, `wait_for_initial_items_with_timeout`, `wait_for_initial_items_from_source`, `match_initial_items_wait_event`, `wait_for_item_with_body`, `wait_for_item_with_body_or_decryption_failure`, `wait_for_bodies_and_pagination_settle`, `find_timeline_item_with_body`, `visit_timeline_diff_items`, `timeline_item_body_matches`; `timeline_item_is_decryption_failure`, `timeline_item_event_id`, `projection_timeline_item`; `QaLogoutAccountExpectation`, `ensure_session_restored_account_key`, `ready_account_key`; `WithheldEventProjectionOrigin`, `WithheldEventTargetOutcome`, `withheld_event_target_outcome`, `withheld_event_target_outcome_in_diffs`; `wait_for_settings_persisted`, `settings_snapshot_matches_link_preview_policy`; the cross-scenario room waits `wait_for_invite_in_snapshot`, `wait_for_room_created`, `wait_for_room_in_room_list`, `wait_for_space_created`, `wait_for_space_child_set`, `wait_for_space_child_projection`, `wait_for_space_in_space_list`, `wait_for_dm_room_in_room_list`, `wait_for_encrypted_room_projection_for_qa`, `wait_for_user_invited`, `wait_for_room_joined`, `space_has_expected_children`, and the owner-adjacent `core_event_kind`;  and generic session/sync/logout/operation-failure deadline waiters. `assert_no_decryption_failure_items` is excluded and remains identity-owned. The owner-local `wait_for_room_unread_count` and `subscribe_and_ack_active_timeline_projection_for_qa` are also excluded and pinned to `scenarios/timeline.rs`. |
| `participants.rs` | `qa_data_dir`; `QaParticipantLoginGate`, `QaParticipantLoginOutcome`, owned participant/session phase types; `authenticated_session_info`, `authenticated_session_info_from_state`; `refresh_device_keys_and_assert_known_for_qa`, `ensure_incoming_verification_receiver_sync_not_stopped`, `wait_for_verification_requested_event_only`, `requested_verification_flow_id`, `verification_state_matches_target`, `verification_state_target`, `wait_for_verification_accepted`, `verification_state_is_at_least_accepted`, and `verification_state_sas`; preserve the reviewed `ensure_incoming...` → `wait_for_verification_requested_event_only` → `requested_verification_flow_id` adjacency for the private-safe source contract; participant login/bootstrap/recovery/identity-gate construction and verification checkpoint helpers; `QaE2eeLogoutBarrier`, `e2ee_cleanup_logout_barrier`, `QaOwnedE2eeCleanupOperations`, `QaCoreOwnedE2eeCleanupOperations`, `cleanup_owned_e2ee_lifecycle_best_effort`, and `cleanup_owned_e2ee_participant_best_effort` own one participant's ordered stop/logout/authoritative-settle/drop/shutdown path. |
| `fixtures.rs` | Cross-scenario fixture construction only: `native_attention_room`, `private_room_options`, `create_room_for_qa`, `create_space_for_qa`, `invite_user_for_qa`, `set_space_child_for_qa`, `start_direct_message_for_qa`, `accept_invite_for_qa`, `load_room_settings_for_qa`, `assert_room_settings_contains_members`, `select_space_and_wait_for_room_scope`, and their owner-adjacent private helpers `wait_for_user_invited_ack`, `wait_for_room_settings_loaded`, `wait_for_invite_accepted`, `wait_for_direct_message_started`, `select_room_list_filter_for_qa`, and `room_list_matches_selected_space`. It imports infrastructure only and no scenario module. Owner-only `create_public_directory_room_for_qa` remains in rooms. |
| `cleanup.rs` | Multi-participant/full-flow aggregation only: `cleanup_e2ee_callers_after_stage_failure`, `cleanup_e2ee_multi_device_participants`, `cleanup_all_owned_e2ee_participants`, `cleanup_normal_secondary_participant_for_qa`, `cleanup_logged_in_runtime`, `cleanup_after_login_sync`, and `cleanup_after_full_flow`. It imports participant lifecycle in one direction; participants never imports cleanup. |
| `diagnostics.rs` | private-safe timeout/category/count formatting; `verification_state_flow_id`; TCP proxy server/handle/request kind/action/message action/canned-page items; forwarding/counter/order helpers. It imports no scenario module. |
...
| `scenarios/timeline.rs` | timeline/reply/thread/reconnect/stress/activity/composer/media/live signals/scheduled send/send queue/cache restore/link preview; owner-local `wait_for_room_unread_count` (only `run_activity_stage` calls it), `subscribe_and_ack_active_timeline_projection_for_qa` (only reconnect calls it), and send-queue-specific observers including `observe_send_queue_retry_item_state`, `wait_for_send_completions_in_order`, `wait_for_cancelled_or_removed_send`, `unsubscribe_timeline_for_qa`, and `assert_zero_display_projection_reset_fallback_delta`. It consumes generic send/body waits from `event_wait.rs`; it does not export them. |
...
**Edge rule:** scenario files never import sibling scenario files. Diagnostics and event-wait never import participants, fixtures, cleanup, or scenarios; participants never imports cleanup, fixtures, or scenarios; cleanup may import participants but never fixtures or scenarios; fixtures may import participants/event-wait but never cleanup or scenarios. All cross-family send/body/deadline/state-wait helpers named above live in `event_wait.rs`; participant/session/verification construction lives in `participants.rs`; room/space/DM fixture construction lives in `fixtures.rs`; lifecycle items live in `cleanup.rs`. Any unlisted helper with callers in more than one scenario owner must be added to this table and re-reviewed before implementation, not decided during integration.

## Source-characterization guard pins

The base contains exactly 49 production-source reads across 49 test functions: 45 `include_str!("headless-core-qa.rs")` reads plus four explicit `CARGO_MANIFEST_DIR` `/src/bin/headless-core-qa.rs` reads. All 49 tests move to `contracts.rs`; none remains owner-local. `contracts::production_source()` concatenates root, registry, diagnostics, event-wait, participants, fixtures, cleanup, orchestrator, and all four scenario production files after trimming each at its owner-local `#[cfg(test)]` boundary. Contract test bodies are therefore absent from the searched text. Tests that require one function body slice use start/end markers from the same final owner file; cross-owner absence/order checks use the concatenated production source. `strict_e2ee_waiter_source` remaps the room-list waiter to event-wait and verification-request/accept waiters to participants; the withheld projection test import moves from timeline to event-wait and participant tests import `requested_verification_flow_id` from participants.

The four external `crates/koushi-core/tests/send_queue_fast.rs` headless-binary guards are pinned as follows: the guards beginning at base lines 1588, 1661, and 1700 read `headless_core_qa/scenarios/timeline.rs`; the classifier/proxy counter guard beginning at 1606 reads `headless_core_qa/diagnostics.rs`. The guard near 1533 reads `send_queue_fast.rs` itself and remains byte-identical; it is not a headless-binary guard. `releaseScripts.test.ts` replaces the single root entry with the explicit 12-file production list (root, registry, diagnostics, event_wait, participants, fixtures, cleanup, orchestrator, identity, rooms, timeline, search), excluding `contracts.rs` and owner test files. `scripts/check-agents-docs.mjs` replaces only its headless scenario-discovery source from `headless-core-qa.rs` to `headless_core_qa/registry.rs`; `real-homeserver-qa.rs`, the scenario regex, and every documentation check remain unchanged.
