...
**Edge rule:** scenario files never import sibling scenario files. Diagnostics and event-wait never import participants, fixtures, cleanup, or scenarios; participants/cleanup never import fixtures or scenarios; fixtures never import scenarios. All cross-family send/body/deadline/state-wait helpers named above live in `event_wait.rs`; participant/session/verification construction lives in `participants.rs`; room/space/DM fixture construction lives in `fixtures.rs`; lifecycle items live in `cleanup.rs`. Any unlisted helper with callers in more than one scenario owner must be added to this table and re-reviewed before implementation, not decided during integration.
