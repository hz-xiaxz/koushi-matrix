...
The base contains exactly 49 production-source reads across 49 test functions: 45 `include_str!("headless-core-qa.rs")` reads plus four explicit `CARGO_MANIFEST_DIR` `/src/bin/headless-core-qa.rs` reads. All 49 tests move to `contracts.rs`; none remains owner-local. `contracts::production_source()` concatenates root, registry, diagnostics, event-wait, participants, fixtures, cleanup, orchestrator, and all four scenario production files after trimming each at its owner-local `#[cfg(test)]` boundary. Contract test bodies are therefore absent from the searched text. Tests that require one function body slice use start/end markers from the same final owner file; cross-owner absence/order checks use the concatenated production source.
