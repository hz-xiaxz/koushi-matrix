...
3. Call the helper only while the authoritative parsed title is `awaitingVerification`. After bootstrap, do not accept the first `session=ready` title alone: the secure-backup gate can still own the DOM briefly. Require the real composer boundary `.composer-inline-editor[role="textbox"]` before returning readiness, using the same loop and deadline. The current composer is an IME-safe contenteditable `div`; the stable class+role contract avoids localized aria-label text and replaces the runner's stale `textarea` selector.
