...
3. Call the helper only while the authoritative parsed title is `awaitingVerification`. After bootstrap, do not accept the first `session=ready` title alone: the secure-backup gate can still own the DOM briefly. Require the real composer boundary `.composer-inline-editor[role="textbox"]` before returning readiness, using the same loop and deadline. The current composer is an IME-safe contenteditable `div`; the stable class+role contract avoids localized aria-label text and replaces the runner's stale `textarea` selector. If the title is ready but the app still shows the default Activity pane, click the first displayed room once and continue waiting; `active_room=true` alone does not prove the room pane owns the DOM.
...
- Add focused source-contract coverage proving:
  ...
  - ready settlement requires both the authoritative ready title and the real contenteditable message-composer DOM boundary, selecting the first room once when the title is ready but the default Activity pane is still visible;
  ...
  - existing release source-contract assertions flip from the textarea literal to the class+role contenteditable literal and assert the stale selector is absent.
