...
- Design: `reviewer-flash` approved the base design and named-edge amendments as `Correct-to-implement`. Warning audit narrowed the approved shared-struct field rule from every field to the ten concretely cross-read fields recorded in the appendix; bounded re-review is pending before visibility cleanup resumes.
...
- Only the appendix's ten concretely cross-read shared-private-struct fields become `pub(super)` to restore the old parent-module scope; every other field remains owner-private and enum variant fields continue following their enum visibility. Test-only use never promotes production visibility.
...
8. The sibling dependency/visibility report matches the appendix's 147 AccountActor method and 65 top-level/type/helper/macro/associated-method cross-owner names exactly; every promoted name and each of the ten shared-struct fields has a concrete edge and no unlisted visibility broadening exists.
...
- exactness cannot prove all 391 production keys, 200 actor methods, 150 tests, five façade names, 33 source sites, 147 sibling methods, 65 shared top-level/type/helper/macro/associated-method names, and the ten shared-private-struct field scopes;
