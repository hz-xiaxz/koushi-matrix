...
| Topic | Decision |
| --- | --- |
| Formats | HTML for reading + `messages.json` (current Element-compatible output, unchanged) as the original |
...
| Architecture | Per-room three-stage pipeline; HTML is a pure function of `messages.json` + `attachments.json` |
...
- Choosing an existing export directory whose manifest matches offers resume:
...
`index.html` for a room is produced by a pure function of `messages.json` and
`attachments.json`. It performs no network access, and the page loads nothing
...
- Each page carries a CSP that allows scripts only from the bundled KaTeX and
...
catalog. CSS is inline, supports light and dark, and prints acceptably.
...
React renders the DTO and dispatches typed commands:
`Start { scope, range, parent_dir }`, `Resume { export_dir }`, `Stop`,
`RetryFailed`, `Dismiss`. The directory picker runs in the Tauri adapter,
which passes the chosen path to Rust.

```
HistoryExportState
  Idle
  Preparing { request_id, scope, range, export_dir }
  Running   { request_id, scope, range, export_dir, rooms, current, stop_requested }
  Completed | Stopped | Failed { request_id, scope, range, export_dir, rooms, failure_kind? }
...
- `Stop` is accepted once, only while `Running`.
- `Resume` and `RetryFailed` are accepted only from `Completed`, `Stopped` or
  `Failed`. `RetryFailed` is a resume over the same export directory.
...
- Start dialog: range, parent directory, number of target rooms and of
  ...
  every attachment. Choosing an existing export directory offers resume.
...
- Because `messages.json` keeps event content verbatim (as Element does),
  ...
  warning covers it.
- KaTeX is MIT-licensed and must stay listed in `THIRD_PARTY_NOTICES.md` now
  that it ships inside exported folders.
