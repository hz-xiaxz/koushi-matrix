...
Status: implementation complete; local CI-equivalent gates green; exact full-diff different-model verdict pending.

Base: `origin/main` / `a03aeb86deab6f3c3bdc8329572ac6b9f215e687`.
Reviewed tree: pending final evidence commit.
...
1. **Core request outcomes / thin adapter (Phase A)**
   ...
   - Session/navigation/search/directory/room/encryption-debug/timeline waiters delegate to Core. Remaining adapter `wait_for_*` names are thin Core wrappers or diagnostic/native platform waits; no production adapter `recv_event` loop remains.
...
- Exact `origin/main...HEAD` different-model full-diff verdict and fixes/recheck.
- Primary integration/self-review verdict on the resulting exact tree.
- Push/PR, hosted CI (including macOS/Windows and required QA lanes), merge, issue/umbrella update, artifact/worktree cleanup.
