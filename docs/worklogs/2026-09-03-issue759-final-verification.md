...
The schema regression passed after the concurrent-gate fix. Fresh hosted runs then isolated avatar completion as the sole remaining failure. Two production-ref experiments did not change the hosted result and were reverted to preserve behavior. The remaining difference was the E2E mock: it observed `download_avatar_thumbnail` synchronously and injected a backend completion in the same browser turn, whereas real Tauri completion is asynchronous. The spec now yields one browser task after observing the request before injecting the account event; product code is unchanged. Exact avatar stress and focused Browser lanes pass locally; a fresh hosted Browser run remains required.
