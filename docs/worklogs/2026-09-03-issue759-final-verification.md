...
The schema regression passed after the concurrent-gate fix. A fresh hosted run then isolated avatar completion as the sole remaining failure: the key-reset effect still cleared in-flight request/retry refs after render relevance was computed. It now retains only entries intersecting the current visible relevance set, preserving same-key completions while pruning cross-key/account stale MXCs. Exact avatar stress passes 10/10, media Vitest passes 26/26, and typecheck passes. Fireworks reviewed the narrowed reset/refinement and returned `CORRECT-TO-MERGE` with no finding.
