...
import { normalizePath, parseQaTitle, qaStatusHasAttentionBaseline, qaStatusHasRequiredPanel, qaStatusHasSendSuccess, qaStatusIsReady, qaWindowStatePathHasContract, requireNonEmptyFile, safeTimestamp, timestamp } from "../evidence.mjs";
import { buildLdPreload, childEnvironment, nssWrapperEnvironment } from "../redaction.mjs";
import { checkLinuxTools, connectOnce, ensureAppBinary, ensureDbusSession, findFreeDisplayNumber, guiScenarioServerKind, qaDataDirForRun, recordProcessOutput, resolveDebugAppBinary, runLoggedCommand, settleChild, sleep, spawnLogged, startDbusMonitor, startXvfb, terminateProcessGroup, triggerNotificationSmoke, validatedQaProfileName, waitForDbusMonitorReady, waitForDbusMonitorToken, waitForDisplaySocket, waitForPort, waitForSignedOutTitle } from "../runtime.mjs";
import { artifactRoot, desktopDir, realLoginFromStdin, timeoutMs } from "../options.mjs";
import { mkdirSync } from "node:fs";
import { join } from "node:path";
import { createRoom, freePort, inviteUser as inviteUserToRoom } from "../../lib/local-homeserver-qa.mjs";
